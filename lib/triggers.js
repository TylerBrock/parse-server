...
var parseModule = global.Parse || _node2.default;
...
function addFunction(functionName, handler, validationHandler, applicationId) {
  applicationId = applicationId || parseModule.applicationId;
...
function getResponseObject(request, resolve, reject) {
  return {
    ...
    error: function error(_error) {
      var scriptError = new parseModule.Error(parseModule.Error.SCRIPT_FAILED, _error);
...
function maybeRunTrigger(triggerType, auth, parseObject, originalParseObject, config) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    parseModule.applicationId = config.applicationId;
    parseModule.javascriptKey = config.javascriptKey || '';
    parseModule.masterKey = config.masterKey;
...
function inflate(data, restObject) {
  ...
  return parseModule.Object.fromJSON(copy);
}